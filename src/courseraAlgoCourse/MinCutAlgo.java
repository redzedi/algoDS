...
public class MinCutAlgo {
	
	public static MyUndirectedGraph buildGraphFromGivenFile(String filePath) throws IOException{
		Reader rdr = null;
		try {
			MyUndirectedGraph.GraphBuilder builder = new MyUndirectedGraph.GraphBuilder();
	...
	public static int getMinCutOfGraphByKargersAlgo(MyUndirectedGraph grph){
		int minCutNum = -1;
		...
		for(int i=0; i < numOfItr_intVal; i++){
			...
			while((numOfAvlablNodes = grph.getNumOfAvailableNodes()) > 2){
				...
				grph.contractEdge();
				//System.out.println("time taken for contract 1 Edge --> "+(System.currentTimeMillis() - t11));
			}
	...
	public static void main(String[] args) throws IOException {
		MyUndirectedGraph grph = buildGraphFromGivenFile("/Users/syadav1/techWork/algoAnalysis/kargerMinCut.txt");
